...
from lib.roi_data_layer.minibatch_single import get_minibatch, get_minibatch
...
class roibatchLoader(data.Dataset):
  def __init__(self, roidb, ratio_list, ratio_index, batch_size,
               num_classes, training=True, normalize=None, im_type=['im', 'mv', 'residual']):
    self._roidb = roidb
    ...
    self.im_type = im_type # types of data to load
  ...
  def __getitem__(self, index):
    ...
    target_size = [self.target_size_batch[index]] # the min edge will be resized to target_size

    blobs = get_minibatch(minibatch_db, self._num_classes, target_size, im_type=self.im_type)
    data = torch.from_numpy(blobs['data']).contiguous()
    im_info = torch.from_numpy(blobs['im_info'])
    # we need to random shuffle the bounding box.
    data_height, data_width, data_channel = data.size(1), data.size(2), data.size(3)
    if self.training:
        ...
        return im_info, padding_data, gt_boxes_padding, num_boxes
    else:
        #print('detect: ', minibatch_db[0]['image'])
        ...
        return im_info, data, gt_boxes, num_boxes
