...
from lib.roi_data_layer.minibatch_pair import get_minibatch, get_minibatch
...
class roibatchLoader(data.Dataset):
  def __init__(self, roidb, ratio_list, ratio_index, batch_size,
               num_classes, group_size = 12, training=True, normalize=None, im_type=['im', 'mv', 'residual']):
    self._roidb = roidb
  ...
  def __getitem__(self, index):
    ...
    if self.training:
        ...
        for j in range(len(blobs['data'])):
            ...
            one_data = torch.from_numpy(blobs['data'][j]).contiguous()
            # if j == 0: # i frame
            #     one_data = one_data[:, :, 0:3] # if this frame is an i frame, we only keep the first 3 channel (i image)
            #                                     # the other channel are all 0
        ...
        i_frame = blobs['data'][0]
        i_frame_box = blobs['boxes'][0]
        i_num_box = blobs['num_boxes'][0]

        p_frame = blobs['data'][1]
        p_frame_box = blobs['boxes'][1]
        p_num_box = blobs['num_boxes'][1]
