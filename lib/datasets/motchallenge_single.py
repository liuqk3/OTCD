...
import PIL
import pickle
from lib.datasets.imdb_single import imdb
import pandas
...
class motchallenge(imdb):
    def __init__(self, phase, name='motchallenge_single', mot_database_path=None, train_ratio=1, interval=2):
        print('loading ' + name + '...')
        ...
        self._2DMOT2015 = {'train': ['ETH-Bahnhof', 'ETH-Sunnyday', 'KITTI-13', 'KITTI-17', 'PETS09-S2L1', 'TUD-Campus', 'TUD-Stadtmitte'],
    ...
    def _load_frame_index(self):
        ...
        for seq in sequences:
            ...
            if seq in MOT15:
                dataset = '2DMOT2015'
                print(self._phase + ': loading ' + dataset + ' ' + seq + '...')
            elif seq in MOT17:
                dataset = 'MOT17'
                print(self._phase + ': loading ' + dataset + ' ' + seq + '...')
            ...
            # we remove the last frame, because the mv and residual in the compressed
            # video of the last frame are all zeros
    ...
    def gt_roidb(self):
        ...
        interval = str(int(self._interval))
        cache_file = os.path.join(self.cache_path, self.name + '_' + train_ratio + '_' + interval + '_gt_roidb.pkl')
    ...
    def _load_mot_annotation(self, index):
        ...
        the index has the format:  2DMOT2015/train/AUD-Campus/img1/000001.jpg
        """
        if self._phase in ['train', 'training', 'val', 'validation']:
            ...
            gt_classes = np.ones((bbox.shape[0]), dtype=np.int32) # has size (num_box,)
            if dataset == 'MOT17':
    ...
    def _mot_eval(self, detpath, classname, ovthresh=0.5, use_07_metric=False):
        ...
        #     anno = self._load_citypersons_annotation(imgindex) # all the boxes are for person
            ...
            #bbox = np.asarray([anno['boxes'][i] for i in range(anno['boxes'].shape[0]) if self._classes[anno['gt_classes'][i]] == classname])
            bbox = []
