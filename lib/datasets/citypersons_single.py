...
class citypersons(imdb):
    def __init__(self, phase, name='citypersons_single', cityperson_database_path=None):
        print('loading ' + name + '...')
        ...
        for city in cities:
            ...
            for im in images: # such as: jena_000078_000019_leftImg8bit.png
                ...
                if im_list[-1] == self._image_ext[1:]:
                    frame_id = 1 # frame_id starts from 1
                    one_index = os.path.join(im_list[0], str(frame_id).zfill(6) + self._image_ext)

                    # check if there exits fg in this image
                    ann_path = self._ann_path_from_index(one_index)
                    im_info_path = self._im_info_path_from_index(one_index)
    ...
    def gt_roidb(self):
        ...
        gt_roidb = [self._load_citypersons_annotation(index)
