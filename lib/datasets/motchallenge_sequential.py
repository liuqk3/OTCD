...
class motchallenge(imdb):
    def __init__(self, phase, name='motchallenge_sequential', mot_database_path=None, train_ratio=1):
    ...
    def _load_mot_annotation(self, index):
        ...
        # # clip the box to image view
        # refined_bbox[:, 0] = refined_bbox[:, 0].clip(0, im_w - 1)
        # refined_bbox[:, 1] = refined_bbox[:, 1].clip(0, im_w - 1)
        # refined_bbox[:, 2] = refined_bbox[:, 2].clip(0, im_h - 1)
        # refined_bbox[:, 3] = refined_bbox[:, 3].clip(0, im_h - 1)
