...
class ImageViewer(object):
    ...
    * 'ESC' : quit
    ...
    def __init__(self, update_ms=20, window_shape=(640, 480), caption="Figure 1"):
    ...
    def rectangle(self, x, y, w, h, label_tl=None, label_br=None):
        """Draw a rectangle.
        ...
        label_tl : Optional[str]
            A text label that is placed at the top left corner of the
            rectangle. Can be used to show the ID of this bbox
        label_br : Optional[str]
            A text label that is placed at the bottom right corner of the
            rectangle. Can be used to show the score of this bbox

        """
        pt1 = int(x), int(y)
        pt2 = int(x + w), int(y + h)
        cv2.rectangle(self.image, pt1, pt2, self._color, self.thickness)
        if label_tl is not None:
            text_size = cv2.getTextSize(
                label_tl, cv2.FONT_HERSHEY_PLAIN, 2, self.thickness)

            center = pt1[0] + 5, pt1[1] + 5 + text_size[0][1]
            pt2 = pt1[0] + 10 + text_size[0][0], pt1[1] + 10 + \
                text_size[0][1]
            cv2.rectangle(self.image, pt1, pt2, self._color, -1)
            #cv2.rectangle(img=self.image, pt1=pt1, pt2=pt2, color=self._color, thickness=self.thickness)
            cv2.putText(self.image, label_tl, center, cv2.FONT_HERSHEY_PLAIN,
                        2, (255, 255, 255), self.thickness)
        if label_br is not None:
            pt2 = int(x + w), int(y + h)
            text_size = cv2.getTextSize(
                label_br, cv2.FONT_HERSHEY_PLAIN, 1, self.thickness)

            center = pt2[0] - 5 - text_size[0][0], pt2[1] - 5
            pt1 = pt2[0] - 10 - text_size[0][0], pt2[1] - 10 - text_size[0][1]

            cv2.rectangle(self.image, pt1, pt2, (0, 0, 0), -1)
            cv2.putText(self.image, label_br, center, cv2.FONT_HERSHEY_PLAIN,
    ...
    def annotate(self, x, y, text, color=None):
        ...
        color: tuple, the color of annotations, (b, g, r)

        """
        if color is None:
            color = self.color
        cv2.putText(self.image, text, (int(x), int(y)), cv2.FONT_HERSHEY_PLAIN,
                    fontScale=5, color=color, thickness=8)#self.thickness)
