...
if __name__ == '__main__':
    ...
    args.arch = 'rfcn_detection'
    args.save_dir = '../save'
    args.net = 'resnet'
    args.num_layer_i = 101
    args.cuda = True if len(GPU_IDS) > 0 else False
    args.mGPUs = True if len(GPU_IDS) > 1 else False
    args.session = 4
    args.batch_size = 2
    args.lr = 1e-3
    args.lr_decay_step = 7
    args.train_module = 'rcnn_base_i'
    ...
    if args.dataset in ['mot', 'motchallenge']:
        pair_or_single = 'single' if args.train_module == 'rcnn_base_i' else 'pair'
        name1 = 'motchallenge_{}_train'.format(pair_or_single)
        ...
        name1 = 'citypersons_{}_train'.format(pair_or_single)
        args.imdb_name = name1
        # in fact, we found that the max_num_gt_boxes in MOT2015 and MOT17 is 52
        args.set_cfgs = ['ANCHOR_SCALES', '[1, 4, 8, 16, 32]', 'ANCHOR_RATIOS', '[1, 3]', 'MAX_NUM_GT_BOXES', '55',
        ...
        pair_or_single = 'single'
        name1 = 'motchallenge_{}_train'.format(pair_or_single)
        name2 = 'citypersons_{}_train'.format(pair_or_single)
    ...
    if args.large_scale:
        args.cfg_file = "../cfgs/{}{}_ls.yml".format(args.net, args.num_layers_i)
    else:
        args.cfg_file = "../cfgs/{}{}.yml".format(args.net, args.num_layer_i)
    ...
    if args.arch == 'rfcn_detection' and args.net == 'resnet':
        from lib.model.detection_net.rfcn import RFCN
        cnn_model = RFCN(classes=imdb.classes, base_net=args.net, num_layers_i=args.num_layer_i,
    ...
    if args.resume:
        load_name = os.path.join(output_dir, '{}_{}{}_{}_{}_{}.pth'.format(args.arch,
                                                                             args.net,
                                                                             args.resume_num_layer_i,
                                                                             args.checksession,
                                                                             args.checkepoch,
                                                                             args.checkpoint))
        #load_name = '../save/rfcn_detection/resnet/motchallenge/rfcn_detection_resnet101_1_15_1158.pth'
        print("loading checkpoint %s" % (load_name))
        checkpoint = torch.load(load_name)
        cnn_model.load_state_dict(checkpoint['model'])#, strict=False)
        # checkpoint['model'] = cnn_model.state_dict()
        # save_checkpoint(checkpoint, '../save/rfcn_detection/resnet/motchallenge+citypersons/rfcn_detection_resnet101_26_10_6929.pth')
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            if 'rcnn_base_i' in args.train_module:
                outputs = cnn_model(im_info, i_frame, i_frame_box, i_num_box)
            elif 'rcnn_base_p' in args.train_module or 'joint' in args.train_module:
                p_frame.data.resize_(data[4].size()).copy_(data[4]).contiguous()
                p_frame_box.data.resize_(data[5].size()).copy_(data[5]).contiguous()
                p_num_box.data.resize_(data[6].size()).copy_(data[6]).contiguous()
                outputs = cnn_model(im_info, i_frame, p_frame, p_frame_box, p_num_box)
            else:
                raise RuntimeError('unrecognized train module: {}'.format(args.train_module))
            ...
            if step % args.disp_interval == 0:
                ...
                print("          rpn_cls | rpn_box | rcnn_cls | rcnn_box |")
                print("  loss    %.5f | %.5f | %.5f  | %.5f  |" \
                      % (loss_rpn_cls, loss_rpn_box, loss_rcnn_cls, loss_rcnn_box))
                print(" weight   %.3f   | %.3f   | %.3f    | %.3f    | "
                      % (args.rpn_loss_cls_weight, args.rpn_loss_box_weight, args.rcnn_loss_cls_weight,
                         args.rcnn_loss_bbox_weight))
                print("loss: %.4f, lr: {}, fg/bg=(%d/%d), time cost: %f".format(change_to_string(np.unique(lr))) % (loss_temp, fg_cnt, bg_cnt, end - start))
                if args.use_tfboard:
                    info = {
                        'loss': loss_temp,
        ...
        if args.mGPUs:
            save_name = os.path.join(output_dir,
                                     '{}_{}{}_{}_{}_{}.pth'.format(args.arch, args.net, args.num_layer_i, args.session, epoch, step))
            save_checkpoint({
